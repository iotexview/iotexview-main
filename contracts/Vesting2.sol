...
/**
 * TXVTeam tokens vesting contract. 
 *
 * According to the TXVTeam token distribution structure, there are two parties that should
 ...
 *
 * The TXVTeam "Vesting" smart contract should be in place to ensure meeting the token sale commitments.
 *
...
contract TXVTokensVesting {
     ...
     * Address of TXVToken.
     */
    ERC20TokenInterface public TXVToken;
     ...
     *
     * @param token Address of TXVToken that will be locked on contract.
    ...
    function getAvailableTokensToWithdraw () public view returns (uint256 tokensToSend) {
        ...
        if (tokensUnlockedPercentage >= 100) {
            tokensToSend = TXVToken.balanceOf(this);
        } else {
    ...
    function setInitialTokensBalance () private {
        initialTokensBalance = TXVToken.balanceOf(this);
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        if (tokensToSend > 0) {
            ...
            TXVToken.transfer(withdrawAddress, tokensToSend);
            // Raising event
...
contract TeamAndEarlyInvestorsVesting is TXVTokensVesting {
    constructor(ERC20TokenInterface token, address withdraw) TXVTokensVesting(token, withdraw) public {}
}